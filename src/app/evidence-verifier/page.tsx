  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-orange-200 rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-slate-500 text-xs font-semibold">Anti-Hallucination & Cryptographic Gazette Audit</span>
          </div>
        ...
        <div className="flex space-x-2">
          <Link href="/" className="bg-orange-600 hover:bg-orange-700 text-white px-3.5 py-2 rounded-lg text-xs font-bold transition flex items-center space-x-1 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Input */}
        <div className="lg:col-span-1 bg-white p-5 rounded-xl border border-orange-200 shadow-sm space-y-4">
          ...
          <textarea 
            ...
            className="w-full bg-orange-50/30 border border-orange-200 text-slate-900 text-xs font-mono rounded-xl p-3 focus:ring-2 focus:ring-orange-500 focus:outline-none"
          ...
          <button
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-xs py-3.5 rounded-xl shadow-md flex items-center justify-center space-x-2 transition"
          >
            {isVerifying ? (
              <>
          ...
          <div className="bg-orange-50 border border-orange-200 p-3 rounded-xl text-xs space-y-1 text-orange-950">
            <span className="font-bold block text-orange-900">&bull; Official Audit Guarantee:</span>
            <p className="text-[11px] text-slate-700">System cross-references exact Gazette PDF byte signatures to ensure 0% hallucination rate.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {!result && !isVerifying && (
            <div className="bg-white rounded-xl p-10 border border-slate-200 text-center space-y-4 shadow-sm">
              <ShieldCheck className="w-16 h-16 text-orange-600 mx-auto animate-pulse" />
          ...
          {result && !isVerifying && (
            <div className="space-y-6">
              ...
              {/* Verdict Header Card */}
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 sm:grid-cols-4 gap-4 items-center">
                
                <div className="sm:col-span-1 border-r border-slate-200 pr-4 text-center">
                  <div className="text-3xl font-black text-orange-600">{result.authenticityScore}%</div>
                  <p className="text-[10px] font-extrabold uppercase text-slate-500 mt-1">Grounding Confidence</p>
                </div>

                <div className="sm:col-span-3 space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="bg-orange-600 text-white font-extrabold text-xs px-3 py-1 rounded-full uppercase">
                      {result.verdict}
                    </span>
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="bg-orange-50/60 border border-orange-200 p-4 rounded-xl text-xs space-y-2 text-slate-900">
                  <strong className="text-orange-900 font-bold block">Verified Clause Match: {result.clauseMatched}</strong>
                  <p className="leading-relaxed font-medium text-slate-700">{result.explanation}</p>
                </div>

                <div className="pt-2 flex items-center justify-between text-xs text-slate-500">
                  <span>Gazette Cryptographic Stamp: <strong>0x8F92A1B3...99C</strong></span>
                  <a href="https://www.services.bis.gov.in" target="_blank" rel="noreferrer" className="text-orange-700 font-bold hover:underline flex items-center space-x-1">
